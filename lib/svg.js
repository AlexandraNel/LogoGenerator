...
class SVG {
    ...
    userInputText(text, textColour) {
        ...
        if (letters) {
            this.text = letters.toSVG();
        }
    };

     // Method to generate the final SVG markup
     generateSVG() {
        return `<svg version="${this.version}" width="${this.width}" height="${this.height}" xmlns="${this.xmlns}" style="background-color: ${this.bgFill};">
            ${this.shape}
            ${this.text}
        </svg>`;
    }
